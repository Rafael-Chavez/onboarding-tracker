...
function App({ onboardings: propOnboardings, refreshOnboardings }) {
  ...
  const onboardings = useMemo(() => {
    return propOnboardings || internalOnboardings;
  }, [propOnboardings, internalOnboardings]);
  ...
  useEffect(() => {
    // Only set up subscription if we are NOT using propOnboardings
    if (!propOnboardings) {
      fetchOnboardings()
