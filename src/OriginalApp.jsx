...
function App({ onboardings: externalOnboardings }) {
  ...
  // Use external onboardings if provided (AdminDashboard), otherwise use internal state
  ...
  // Optimized lookup by pre-indexing onboardings by date
  const onboardingsByDate = useMemo(() => {
    const index = new Map();
    onboardings.forEach(ob => {
      if (!index.has(ob.date)) {
        index.set(ob.date, []);
      }
      index.get(ob.date).push(ob);
    });
    return index;
  }, [onboardings]);
