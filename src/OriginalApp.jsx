...
function App({ onboardings: externalOnboardings, subscribeToOnboardings }) {
  ...
  const [onboardings, setOnboardings] = useState(externalOnboardings || [])
  ...
  // Only use internal state/subscription if external onboardings are not provided
  ...
  useEffect(() => {
    if (externalOnboardings) {
      setOnboardings(externalOnboardings);
    }
  }, [externalOnboardings]);

  // Load onboardings from Supabase on mount if not provided externally
  useEffect(() => {
    if (externalOnboardings) return;
  ...
  }, [onboardingsByMonth, onboardings.length])
