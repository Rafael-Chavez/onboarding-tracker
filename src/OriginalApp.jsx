...
function App({ onboardings: propOnboardings }) {
  ...
  const [onboardings, setOnboardings] = useState(propOnboardings || [])
  ...
  useEffect(() => {
    if (propOnboardings) {
      setOnboardings(propOnboardings)
    }
  }, [propOnboardings])

  // Load onboardings from Supabase on mount (only if not provided as prop)
  useEffect(() => {
    if (!propOnboardings) {
      fetchOnboardings()
  ...
  const addOnboarding = useCallback(async (formData) => {
    const { clientName, accountNumber, selectedEmployee: formEmployeeId } = formData;

    if (formEmployeeId && clientName.trim() && accountNumber.trim()) {
      ...
      const newOnboarding = {
        ...
        employeeName: employees.find(e => e.id === parseInt(formEmployeeId))?.name,
