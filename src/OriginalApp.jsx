...
function App() {
  ...
  const onboardingsByDate = useMemo(() => {
    ...
    onboardings.forEach(ob => {
      if (!map[ob.date]) map[ob.date] = [];
      map[ob.date].push(ob);
    });
    return map;
  }, [onboardings]);

  const addOnboarding = useCallback(async (clientName, accountNumber, notifyEmployee = false) => {
    if (selectedEmployee && clientName.trim() && accountNumber.trim()) {
      ...
      if (result.success) {
        ...
        if (notifyEmployee && employee) {
          EmailNotificationService.sendCustomEmail({
            to: EmailNotificationService.ADMIN_EMAIL,
            subject: `New Onboarding Assigned: ${clientName}`,
            body: `
New Onboarding Scheduled
=======================
