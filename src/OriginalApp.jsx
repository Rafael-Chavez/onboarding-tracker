...
function App() {
  ...
  // Pre-index onboardings by date for O(1) calendar lookups
  ...
  const addOnboarding = useCallback(async ({ employeeId, clientName, accountNumber, notifyEmployee }) => {
    ...
    if (result.success) {
      ...
      // Send email notification to employee if requested
      if (notifyEmployee) {
        const employee = employees.find(e => e.id === parseInt(employeeId));
        if (employee) {
          EmailNotificationService._sendEmailViaBackend({
            to: EmailNotificationService.ADMIN_EMAIL, // For now sending to admin as per service design
            subject: `New Onboarding Assigned: ${clientName}`,
            body: `
Hello ${employee.name},

You have been assigned a new onboarding session.

Details:
- Client: ${clientName}
- Account: ${accountNumber}
- Date: ${new Date(newOnboarding.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
- Session: #${newOnboarding.sessionNumber}
