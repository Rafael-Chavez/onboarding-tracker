  ...
  const getMonthlyCompletionStats = useCallback((date) => {
    ...
    onboardings.forEach(ob => {
      if (ob.month === monthStr) {
        ...
        if (!employeeStats[ob.employeeId]) {
          employeeStats[ob.employeeId] = { total: 0, completed: 0 }
        }
        employeeStats[ob.employeeId].total++
        if (ob.attendance === 'completed') {
          employeeStats[ob.employeeId].completed++
        }
      }
    })

    const byEmployee = employees.map(emp => {
      const stats = employeeStats[emp.id] || { total: 0, completed: 0 }
      return {
        ...emp,
        ...stats
      }
