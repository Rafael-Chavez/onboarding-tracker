...
function App({ onboardings: onboardingsFromProps, subscribeToOnboardings }) {
  ...
  const onboardings = onboardingsFromProps || onboardingsState
  ...
  const fetchOnboardings = useCallback(async () => {
    ...
    if (result.success) {
      setOnboardingsState(result.onboardings)
    } else {
      console.error('Error loading onboardings:', result.error)
    }
  }, []);

  // Load onboardings from Supabase on mount (only if not provided by props)
  useEffect(() => {
    if (onboardingsFromProps) return;

    fetchOnboardings()

    // Subscribe to real-time changes
    const subscription = (subscribeToOnboardings || SupabaseService.subscribeToOnboardings)((payload) => {
  ...
  }, [fetchOnboardings, onboardingsFromProps, subscribeToOnboardings])
  ...
  // Optimized O(1) lookup for onboardings by date
