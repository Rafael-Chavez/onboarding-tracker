...
function App({ onboardingsProp }) {
  ...
  // Update onboardings when prop changes
  useEffect(() => {
    if (onboardingsProp) {
      setOnboardings(onboardingsProp)
    }
  }, [onboardingsProp])

  const fetchOnboardings = useCallback(async () => {
    if (onboardingsProp) return; // Don't fetch if provided via props
  ...
  // Load onboardings from Supabase on mount if not provided as prop
  useEffect(() => {
  ...
  // Pre-index onboardings by date string for O(1) lookups
