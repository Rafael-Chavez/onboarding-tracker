...
const OnboardingForm = ({
  ...
  // Use local state for form inputs to prevent full app re-renders on every keystroke
  // This significantly improves Interaction to Next Paint (INP)
  ...
  const handleSubmit = useCallback(async () => {
    if (!localEmployee || !localClientName.trim() || !localAccountNumber.trim()) return;

    setIsSubmitting(true);
    try {
      // Create the data object to send to the parent
      const formData = {
      ...
      // Call the parent's addOnboarding function
      // We assume the parent will handle the actual data persistence
      const result = await addOnboarding(formData);
  ...
  }, [localEmployee, localClientName, localAccountNumber, addOnboarding]);
    ...
    <div className="backdrop-blur-md bg-white/10 rounded-2xl border border-white/20 p-6 shadow-2xl">
      ...
      <div className="space-y-4">
        ...
        <button
          ...
          disabled={isSubmitting || !localEmployee || !localClientName.trim() || !localAccountNumber.trim()}
