...
export default function EmailNotificationViewer() {
  ...
  const checkSMTP = useCallback(async () => {
    ...
    setTestEmailStatus({
      ...
      message: result.success ? 'SMTP Connection OK' : `SMTP Failed: ${result.error || result.message}`
    });
                  ...
                  >
                    ...
                    {notification.details && (
                      <div className="mt-2 pt-2 border-t border-white/5">
                        <div className="text-[10px] text-white/40 uppercase tracking-widest mb-1">Diagnostic Info</div>
                        <div className="text-[10px] text-blue-300 font-mono bg-blue-950/30 rounded p-1.5 overflow-x-auto">
                          {notification.details.response && (
                            <div className="mb-1">Response: {notification.details.response}</div>
                          )}
                          {notification.details.accepted?.length > 0 && (
                            <div className="text-green-400">Accepted: {notification.details.accepted.join(', ')}</div>
                          )}
                          {notification.details.rejected?.length > 0 && (
                            <div className="text-red-400">Rejected: {notification.details.rejected.join(', ')}</div>
                          )}
                        </div>
                      </div>
                    )}
