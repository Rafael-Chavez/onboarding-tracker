...
const verifyToken = async (req, res, next) => {
  try {
    ...
    try {
      // Verify the token using Firebase Admin if possible
      ...
      // In development mode, fallback to decoding unverified base64 ID token if Firebase Admin fails or is unconfigured
      if (process.env.NODE_ENV === 'development' || !process.env.NODE_ENV) {
        console.warn('Firebase ID Token verification failed, falling back to base64 decoding in development environment.');
        try {
          ...
          console.error('Failed to decode unverified token fallback:', decodeError);
        }
      }
      throw fbError;
    }
